...
class HTML2PDF extends FPDF
{
    ...
    function __construct($orientation='P',$unit='mm',$format='A4',$_title="",$_url="",$_debug=false)
    {
        ...
        $this->debug=$_debug;
        $this->AliasNbPages();
        $this->footerLink = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://{$_SERVER["HTTP_HOST"]}{$_SERVER["REQUEST_URI"]}";
            ...
            if (!$skip) {
                ...
                if($i%2==0)
                {
                    ...
                    if (substr(trim($e),0,1)=='/'){
                        // echo($e);
                        $this->CloseTag(strtoupper(preg_replace("/\//","",$e)));
                    } else {
                        //Extract attributes
                        $a2=explode(' ',$e);
    ...
    function Footer()
    {
        ...
        $this->Cell(0,4,$this->footerText,0,0,'C',0,$this->footerLink);
        $this->mySetTextColor(-1);
    }
