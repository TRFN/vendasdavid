LWDKExec(function(){
    ...
    const getSlideData = window.getSlideData = (() => {
        ...
        slides = MapTranslate(slides, ["categoria","titulo","data","img"]);
    ...
    const setSlideData = window.setSlideData = ((data) => {
        ...
        for(i of data){
            ...
            (!(typeof i != "object" || typeof i.img != "string" || i.img.length < 4)) && $("#gallery").append(
                `<div class='col-4 text-center slide'>
					<label class='col-8'>Titulo:
						<input class='form-control form-control-sm' type=text value="${i.categoria}" />
					</label>
					...
					<label class='col-8'>Data:
                        <input style='text-transform: uppercase;' class='form-control form-control-sm' type=text value="${i.data}" />
                    </label>
